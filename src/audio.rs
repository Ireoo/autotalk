...
impl AudioCapture {
    ...
    pub fn start_capture(&mut self, tx: mpsc::Sender<Vec<f32>>) -> Result<()> {
        ...
        // 尝试不同的配置优先级
        // 首先尝试使用我们预设的通道数和采样率
        let mut selected_config = config_vec.iter()
            .find(|config| {
        ...
        if selected_config.is_none() {
            ...
            selected_config = config_vec.iter()
                .filter(|config| config.channels() == self.channels)
                .min_by_key(|config| {
                    let min_diff = (config.min_sample_rate().0 as i32 - self.sample_rate as i32).abs();
                    let max_diff = (config.max_sample_rate().0 as i32 - self.sample_rate as i32).abs();
        ...
        let config = match selected_config {
            ...
            None => {
                return Err(anyhow::anyhow!("无法为该设备找到兼容的音频配置，请尝试其他麦克风设备"));
            }
        };
